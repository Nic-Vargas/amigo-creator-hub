...
import { movimientos, LEYES } from "@/lib/mock-data";
...
const tipoStyles: Record<string, { label: string; color: string }> = {
  SALDO_INICIAL: { label: "Saldo Inicial", color: "bg-primary/15 text-primary border-primary/30" },
  INCREMENTO: { label: "Incremento", color: "bg-info/15 text-info border-info/30" },
  REINTEGRO: { label: "Reintegro", color: "bg-accent/15 text-accent border-accent/30" },
  NO_PROCEDE: { label: "No Procede", color: "bg-warning/15 text-warning border-warning/30" },
  AJUSTE: { label: "Ajuste", color: "bg-muted text-muted-foreground border-border" },
};
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">Registro de saldos iniciales, incrementos, reintegros y ajustes</p>
        </div>
            ...
            {filtered.map((m, i) => {
              ...
              return (
                <tr key={m.id} className="border-b border-border last:border-0 hover:bg-muted/50 transition-colors animate-fade-in" style={{ animationDelay: `${i * 30}ms` }}>
                  ...
                  <td className="p-3 text-right font-mono text-xs">{m.valorSalud > 0 ? formatCurrency(m.valorSalud) : '—'}</td>
                  <td className="p-3 text-right font-mono text-xs">{m.valorPension > 0 ? formatCurrency(m.valorPension) : '—'}</td>
                  <td className="p-3 text-right font-mono text-xs">{m.valorCuotaMonetaria > 0 ? formatCurrency(m.valorCuotaMonetaria) : '—'}</td>
                  <td className="p-3 text-right font-mono text-xs">{m.valorTransferencia > 0 ? formatCurrency(m.valorTransferencia) : '—'}</td>
                  <td className="p-3 text-right font-mono font-semibold">{formatCurrency(m.valor)}</td>
