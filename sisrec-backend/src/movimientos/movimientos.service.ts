...
export class MovimientosService {
  ...
  async create(dto: CreateMovementDto, user: CurrentUserPayload) {
    this.validateDetails(dto);

    return this.prisma.$transaction(async (tx) => {
      const recobro = await tx.recobroCase.findFirst({
        where: {
      ...
      const caseTotalAnterior = Number(recobro.valorTotal);
      const beneficiaryTotalAnterior = Number(
        recobro.beneficiary.saldoTotal,
      );

      const caseUpdateData: Record<string, number | Date> = {
      ...
      for (const detail of dto.detalles) {
        const conceptField = this.getConceptField(detail.concepto);

        const saldoAnteriorConcepto = Number(recobro[conceptField]);

        const saldoNuevoConcepto = this.calculateNewBalance({
          tipo: detail.tipo,
          valor: detail.valor,
          saldoActual: saldoAnteriorConcepto,
          adjustmentDirection: detail.adjustmentDirection,
        });

        const delta = saldoNuevoConcepto - saldoAnteriorConcepto;

        totalDelta += delta;
        caseUpdateData[conceptField] = saldoNuevoConcepto;

        movementDetails.push({
          tipo: detail.tipo,
          concepto: detail.concepto,
          adjustmentDirection: detail.adjustmentDirection ?? null,
          valor:
            detail.tipo === MovementType.NO_PROCEDE
      ...
      const caseTotalNuevo = caseTotalAnterior + totalDelta;
      ...
      caseUpdateData.valorTotal = caseTotalNuevo;

      const updatedCase = await tx.recobroCase.update({
        where: {
          id: recobro.id,
        },
        data: caseUpdateData,
      ...
      const aggregate = await tx.recobroCase.aggregate({
        where: {
          companyId: user.companyId,
          beneficiaryId: recobro.beneficiaryId,
        },
        _sum: {
          valorTotal: true,
        },
      ...
      return {
        movement,
        recobroCase: updatedCase,
        beneficiary: updatedBeneficiary,
      };
    });
  }

  private validateDetails(dto: CreateMovementDto) {
    ...
    const concepts = dto.detalles.map((detail) => detail.concepto);
    const uniqueConcepts = new Set(concepts);

    if (uniqueConcepts.size !== concepts.length) {
      throw new BadRequestException(
    ...
    for (const detail of dto.detalles) {
      ...
      if (
        detail.tipo === MovementType.AJUSTE &&
  ...
  private getConceptField(concepto: MovementConcept) {
    const map = {
      [MovementConcept.SALUD]: 'valorSalud',
      [MovementConcept.PENSION]: 'valorPension',
      [MovementConcept.CUOTA_MONETARIA]: 'valorCuotaMonetaria',
  ...
  private calculateNewBalance(params: {
    ...
    const { tipo, valor, saldoActual, adjustmentDirection } = params;
    ...
    switch (tipo) {
      ...
      case MovementType.AJUSTE:
        ...
        if (adjustmentDirection === AdjustmentDirection.SUMA) {
          return saldoActual + valor;
        }
